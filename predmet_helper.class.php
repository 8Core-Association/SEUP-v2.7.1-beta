...
class Predmet_helper
{
    ...
    /**
     * Insert new predmet with custom date support
     */
    public static function insertPredmet($db, $klasa_br, $sadrzaj, $dosje_br, $god, $rbr_predmeta, $naziv_predmeta, $id_ustanove, $id_zaposlenik, $id_klasifikacijske_oznake, $vrijeme_cuvanja, $posiljatelj_naziv = null, $zaprimljeno_datum = null, $datum_otvaranja = null)
    {
        // Determine timestamp value - use custom date if provided, otherwise NOW()
        $timestamp_value = $datum_otvaranja ? "'" . $db->escape($datum_otvaranja) . "'" : "NOW()";
    ...
    public static function fetchUploadedDocuments($db, $conf, &$documentTableHTML, $langs, $caseId)
    {
        ...
        if (count($documents) > 0) {
            ...
            $documentTableHTML .= '<th><i class="fas fa-weight me-2"></i>&nbsp;Veličina</th>';
            $documentTableHTML .= '<th><i class="fas fa-calendar me-2"></i>&nbsp;Datum</th>';
            $documentTableHTML .= '<th><i class="fas fa-user me-2"></i>&nbsp;Kreirao</th>';
            $documentTableHTML .= '<th><i class="fas fa-certificate me-2"></i>&nbsp;Digitalni potpis</th>';
            ...
            foreach ($documents as $doc) {
                ...
                $documentTableHTML .= '<tr class="' . $rowClass . '">';
